...
namespace MyRep
{
    public partial class RfmdFormPrev : Form
    {
        private string rptFileName; // ✅ Stores the report filename dynamically
        public RfmdFormPrev(string reportFileName)
        {
            InitializeComponent();
            this.rptFileName = reportFileName;
        }

        private void RfmdFormPrev_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!reportDataset.Tables.Contains("RfmdRecSummary"))
                {
                    MessageBox.Show("Error: `RfmdRecSummary` table not found in XML!", "Load Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                // ✅ Extract the `RfmdRecSummary` table from the dataset
                DataTable reportTable = reportDataset.Tables["RfmdRecSummary"];
                ...
                this.crystalReportViewer1.ReportSource = this.rfmdPrint1;
        ...
        private void rfmdPrint1_InitReport(object sender, EventArgs e)
        {
