...
namespace MyRep
{
    public partial class SysSTRptPrev9 : Form
    {
        private string rptFileName; // ✅ Stores the report filename dynamically
        public SysSTRptPrev9(string reportFileName)
        {
            InitializeComponent();
            this.rptFileName = reportFileName;
        }

        private void SysSTRptPrev9_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!reportDataset.Tables.Contains("SysSThruReport9"))
                {
                    MessageBox.Show("Error: `SysSThruReport9` table not found in XML!", "Load Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                // ✅ Extract the `SThruReport` table from the dataset
                DataTable reportTable = reportDataset.Tables["SysSThruReport9"];
                ...
                {
                    MessageBox.Show("Error: No data available in `SysSThruReport9` table!", "Data Load Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                // ✅ Load external report when available; otherwise use embedded report
                ReportRuntimeHelper.TryLoadReport(this.reportDocument6, reportPath);
                ...
                this.reportDocument6.SetDataSource(reportDataset);

                // ✅ Assign the report to the Crystal Report Viewer and refresh it
                this.crystalReportViewer1.ReportSource = this.reportDocument6;
        ...
        private void reportDocument6_InitReport(object sender, EventArgs e)
        {
