...
namespace MyRep
{
    public partial class SysSTRptPrev : Form
    {
        private string rptFileName; // ✅ Stores the report filename dynamically
        public SysSTRptPrev(string reportFileName)
        {
            InitializeComponent();
            this.rptFileName = reportFileName;
        }

        private void SysSTRptPrev_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!reportDataset.Tables.Contains("SysSThruReport2"))
                {
                    MessageBox.Show("Error: `SysSThruReport2` table not found in XML!", "Load Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                // ✅ Extract the `SThruReport` table from the dataset
                DataTable reportTable = reportDataset.Tables["SysSThruReport2"];
                ...
                {
                    MessageBox.Show("Error: No data available in `SysSThruReport2` table!", "Data Load Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                // ✅ Load external report when available; otherwise use embedded report
                ReportRuntimeHelper.TryLoadReport(this.reportDocument4, reportPath);
                ...
                this.reportDocument4.SetDataSource(reportDataset);

                // ✅ Assign the report to the Crystal Report Viewer and refresh it
                this.crystalReportViewer1.ReportSource = this.reportDocument4;
        ...
        private void reportDocument4_InitReport(object sender, EventArgs e)
        {
