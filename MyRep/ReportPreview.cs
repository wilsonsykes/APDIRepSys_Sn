...
namespace MyRep
{
    public partial class ReportPreview : Form
    {
        ...
        public ReportPreview(string reportFileName)
        {
            InitializeComponent();
        ...
        private void ReportPreview_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string xmlPath = Path.Combine(appDataDirectory, "help.xml");
                ...
                if (!reportDataset.Tables.Contains("SThruReport"))
                {
                    MessageBox.Show("Error: `SThruReport` table not found in XML!", "Load Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                // ✅ Extract the `SThruReport` table from the dataset
                DataTable reportTable = reportDataset.Tables["SThruReport"];
                ...
                {
                    MessageBox.Show("Error: No data available in `SThruReport` table!", "Data Load Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                // Try to load an external report file when present.
                // If none is found, Crystal uses the embedded report resource.
                ReportRuntimeHelper.TryLoadReport(this.reportDocument1, reportPath);
                ...
                this.reportDocument1.SetDataSource(reportDataset);
