...
{
    public partial class SysSTRpt3 : Form
    {
        ...
        private void SysSTRpt3_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                this.syssThruReport3TableAdapter1.Connection.ConnectionString = global::APDIRepSys.DatabaseConnectionHelper.GetOdbcConnectionString();
                this.syssThruReport3TableAdapter1.Fill(this.dataSet12.SysSThruReport3);
                ...
                dataGridView1.Columns["nextscheddateDataGridViewTextBoxColumn"].DefaultCellStyle.Format = "MM/dd/yy";
                ...
                dataGridView1.Columns["mos3revenueDataGridViewTextBoxColumn"].DefaultCellStyle.Format = "N2";
                dataGridView1.Columns["mos3revenueDataGridViewTextBoxColumn"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                dataGridView1.Columns["mos3invtyamountDataGridViewTextBoxColumn"].DefaultCellStyle.Format = "N2";
                dataGridView1.Columns["mos3invtyamountDataGridViewTextBoxColumn"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                ...
                DataView dv = new DataView(this.dataSet12.SysSThruReport3);
                ...
                try
                {
                    ...
                    ";
                            ...
                            using (NpgsqlCommand cmd = new NpgsqlCommand(updateQuery, conn, transaction))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@product", product); // ✅ Corrected to use the right column name
                                cmd.Parameters.AddWithValue("@store_group", StoreGroupName);
                                cmd.ExecuteNonQuery();
                            }

                            // 🔁 Update UI immediately
                            row.Cells["storegroupDataGridViewTextBoxColumn"].Value = StoreGroupName; // ✅ Ensure the original column is also updated
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string reportFileName = "SysSellThruRpt3.rpt"; // ✅ Name of the Crystal Report file
                ...
                System.Data.DataView dv = new System.Data.DataView(this.dataSet12.SysSThruReport3);
                ...
                // ✅ Clone full schema so all columns (including order_amount) are retained
                System.Data.DataTable filteredTable = this.dataSet12.SysSThruReport3.Clone();

                // ✅ Import filtered rows into schema-preserved table
                ...
                if (filteredTable.Columns.Contains("order_amount"))
                {
                    foreach (DataRow row in filteredTable.Rows)
                    {
                        ...
                        {
                            row["order_amount"] = DBNull.Value; // ✅ Ensures <order_amount xsi:nil="true" />
                        }
                    }
                }


                filteredTable.TableName = "SysSThruReport3"; // ✅ Ensure MyRep.exe expects this exact table name
                ...
                try
                {
                    ...
                    using (NpgsqlTransaction transaction = conn.BeginTransaction())
                    {
                        ...
                        foreach (DataGridViewRow row in dataGridView1.Rows)
                        {
                            ...
                            int mos3ClosingInvty = currentInvty - mos3ItemSold;
                            decimal mos3InvtyAmt = mos3ClosingInvty * currentPrice;
                            ...
                            UPDATE system_sellthru_summary SET
                                ...
                                mos3_closing_invty = @mos3_closing_invty,
                                mos3_invty_amount = @mos3_invty_amount,
                                order_qty = @order_qty
                            WHERE sellthru_no = @sellthru_no AND product = @product;
                        ";
                            ...
                            using (NpgsqlCommand cmd = new NpgsqlCommand(updateQuery, conn, transaction))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@mos3_closing_invty", mos3ClosingInvty);
                                cmd.Parameters.AddWithValue("@mos3_invty_amount", mos3InvtyAmt);
                            ...
                            row.Cells["mos3closinginvtyDataGridViewTextBoxColumn"].Value = mos3ClosingInvty;
                            row.Cells["mos3invtyamountDataGridViewTextBoxColumn"].Value = mos3InvtyAmt;
                            ...
                            row.Cells["mos3invtyamountDataGridViewTextBoxColumn"].Style.BackColor = Color.Yellow;
                            row.Cells["mos3invtyamountDataGridViewTextBoxColumn"].Style.BackColor = Color.Yellow;
                        }
        ...
        private void FilterDataGridViewBySellThru(string sellThruNo)
        {
            if (string.IsNullOrWhiteSpace(sellThruNo))
            {
                ...
                dataGridView1.DataSource = dataSet12.SysSThruReport3;
                return;
            }

            
            DataView dv = new DataView(dataSet12.SysSThruReport3);
